...
def make_dolphin_win_build(build_type, mode="normal"):
    ...
    f.addStep(Compile(command=["msbuild.exe", "/v:m", "/p:Platform=x64",
                      ...
                      haltOnFailure=True))
    f.addStep(Test(command=["msbuild.exe", "/v:m", "/p:Platform=x64",
                               "/p:Configuration=%s" % build_type,
                               "/p:RunUnitTests=true",
                               "dolphin-emu.sln"],
                      env=env,
    ...
    f.addStep(ShellCommand(command=["C:\\buildbot\\signbin.bat", "Binary\\x64\\%s.exe" % dolphin_name],
                           logEnviron=False,
                           description="signing binary",
                           descriptionDone="sign binary"))

    f.addStep(ShellCommand(command=["xcopy", "Binary\\x64",
                                    "Dolphin-x64", "/S", "/I", "/Y"],
                           logEnviron=False,
                           description="copying output",
                           descriptionDone="output copy"))

    out_filename = WithProperties("Dolphin-%s-%s-x64.7z", "branchname", "shortrev")
    ...
    f.addStep(ShellCommand(command=["rmdir", "/S", "/Q", "Dolphin-x64"],
                           logEnviron=False,
...
def make_fifoci_win(type, mode="normal"):
    ...
    f.addStep(ShellCommand(command=WithProperties("powershell C:/utils/bin/wgetwrapper.ps1 %s build.7z", "build_url"),
...
BuildmasterConfig = {
    ...
    "builders": [
        BuilderConfig(name="release-win-x64", workernames=["windows"],
                      factory=make_dolphin_win_build("Release", "normal,fifoci_golden")),
        ...
        BuilderConfig(name="wip-win-x64", workernames=["windows"],
                      factory=make_dolphin_win_build("Release", "wip")),
        ...
        BuilderConfig(name="pr-win-x64", workernames=["windows"],
                      factory=make_dolphin_win_build("Release", "pr,fifoci_golden")),
        BuilderConfig(name="pr-win-dbg-x64", workernames=["windows"],
                      factory=make_dolphin_win_build("Debug", "pr,debug")),
        ...
        BuilderConfig(name="debug-win-x64", workernames=["windows"],
                      factory=make_dolphin_win_build("Debug", "normal,debug")),
        ...
        BuilderConfig(name="fifoci-ogl-lin-mesa", workernames=["hive"],
